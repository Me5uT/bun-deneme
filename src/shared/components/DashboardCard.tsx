...
export const DashboardCard: React.FC<IDashboardCardProps> = ({ title, items, loading }) => (
      ...
      {items.map(item => (
        <Row key={item.key} align="middle" className="dashboard-card-row">
          ...
          <Col span={16} style={{ display: 'flex', alignItems: 'center' }}>
            {typeof item.children === 'string' ? <span className="dashboard-card-text">{item.children}</span> : item.children}
          </Col>
        </Row>
      ))}
    </Space>
