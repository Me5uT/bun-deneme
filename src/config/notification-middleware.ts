...
const addErrorAlert = (message, key?, data?) => {
  key = key ? key : message;
  console.error(message, key, data);
};
...
export default () => (next) => (action) => {
  ...
  if (isFulfilledAction(action) && payload && payload.headers) {
    ...
    if (alert) {
      ...
      console.error("isFulfilledAction", alertParam);
    }
  }
  ...
  if (isRejectedAction(action) && error && error.isAxiosError) {
    if (error.response) {
        ...
        switch (response.status) {
          ...
          case 400: {
            ...
            if (errorHeader) {
              ...
              for (const fieldError of fieldErrors) {
                ...
                // const fieldName = translate(`mirketPortalMainApp.${fieldError.objectName}.${convertedField}`);
              ...
              console.error(
                "isRejectedAction",
                data?.title || data?.message || data?.error || "Unknown error!"
              );
            }
            break;
          }
          ...
          default:
            if (typeof data === "string" && data !== "") {
              ...
              console.error(
                "isRejectedAction",
                data?.status || data?.title || "Unknown error!"
              );
            }
        }
      }
      ...
      console.error("isRejectedAction", error.message || "Unknown error!");
    }
  } else if (error) {
    // toast.error(error.message || 'Unknown error!');
    console.error("isRejectedAction", error.message || "Unknown error!");
